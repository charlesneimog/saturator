...
void saturator_autogain(t_saturator_tilde *x, t_float f) { x->enableAutogain = f == 0.0 ? 0 : 1; }
...
static void saturator_quant_set(t_saturator_quant *q, t_symbol *type, t_atom *param1) {
    if (strcmp(GETSTRING(type), "linear") == 0) {
        ...
        if (param1 != NULL) {
            ...
            pd_error(q, "saturator~: Mising param 1 for quant type 'pow'");
        }
...
void saturator_debug(t_saturator_tilde *x) {
    post("THRESH: %f, ATTACK: %f, UP: %f, RELEASE: %f, DOWN: %f, QUANT: %f", x->thresh, x->attack->duration,
         x->attack->inc_rmspms, x->release->duration, x->release->inc_rmspms, x->factor);
}
...
void saturator_tilde_setup(void) {
    saturator_tilde_class =
        class_new(gensym("saturator~"), (t_newmethod)saturator_tilde_new, (t_method)saturator_tilde_free,
